...
export class UserMiddleware {
  ...
  static async createNewUser(params: IUsers) {
    ...
    const ret =  await Database.execute('INSERT INTO users (username, email, passowrd_hash) VALUES (?, ?, ?)', [username, email, password_hash]);
    console.log({ret})
    return ret;
  }

  static async getUserByUsername(username:string): Promise<IUsers[]> {
    return await Database.query('SELECT * FROM users WHERE username = ?', [username]);
  }
  ...
  static async isValidUser(username:string, password:string) {
    try {
      ...
      if (user) {
        return await UserMiddleware.verifyPassword(password, user.password_hash);
      }
      return false;
    } catch(e){
      console.log(e);
