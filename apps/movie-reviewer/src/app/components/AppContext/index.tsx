...
export const AuthProvider = (props:IAuthProvider) => {
  ...
  const contextValue = useMemo(() => ({ isLoggedIn, userId, register, login }), [isLoggedIn, userId, register, login]);
...
export const withAppContextContainer = <ComponentProps extends object>(
  ...
  const WithAppContextContainer: React.FC<ComponentProps & IWithAppContextContainerProps> = (props) => {
    return (
      <AuthProvider>
        <AppContainer>
          <Navigation />
